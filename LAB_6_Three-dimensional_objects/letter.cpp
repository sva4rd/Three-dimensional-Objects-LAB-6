...
Letter::Letter(QWidget *parent): QGLWidget(parent)
{
    setGeometry(400,200,800,600);

    defaultScene();

    RotationArray = new GLfloat*[vertices];
    for (int i = 0; i < vertices; i++)
        RotationArray[i] = new GLfloat[3];
...
void Letter::mousePressEvent(QMouseEvent* pe) // нажатие клавиши мыши
{
...
void Letter::mouseMoveEvent(QMouseEvent* pe) // изменение положения стрелки мыши
{
...
void Letter::wheelEvent(QWheelEvent* pe) // вращение колёсика мыши
{
...
{
    xRot=0; yRot=0; zRot=0; zTra=0; nSca=0.7;
}
...
void Letter::getVertexArray()
{
    ...
    else
    {
        for(int i = 0; i < vertices; i++) {
            for(int j = 0; j < 3; j++) {
                VertexArray[i][j] = vertexData[i][j];
                RotationArray[i][j] = vertexData[i][j];
            }
        }
    }
}
    ...
    for(int i=0; i < vertices; i++)
    {
        for(int j=0; j < 3; j++)
        {
            ...
            for(int k=0; k < 3; k++)
            {
                ...
                element += RotationArray[i][k]*conv[k][j];
            }
            VertexArray[i][j] = element;
        }
    }
    drawFigure();
...
void Letter::draw_rotation_y()
{
    ...
    for(int i=0; i < vertices; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
            RotationArray[i][j] = VertexArray[i][j];
    }
    drawFigure();
